...
- **Launcher windows no longer pile up.** Restarting the bot used to leave its old console window sitting open; over a day these "empty shell" windows accumulated. Each start now clears this server's leftover empty launcher windows — strictly folder-scoped, so a second/third server on the same machine is never touched, and any window still running a live bot/web-CC is left alone.
<!-- NIGHTLY-NOTES:END -->
