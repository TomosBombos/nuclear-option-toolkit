#!/usr/bin/env python3
"""Nuke Option Server Toolkit — opt-in GitHub updater (plugin, bot, web CC).
...
component for the next deploy; the owner decides when to apply.
    ...
    python updater.py check                         # what's available to install?
...
Updates come from full GitHub releases only (pre-releases are ignored). For a private
repo, set GITHUB_TOKEN so the updater can read the releases API.
...
def _repo():
    ...
    return (upd.get("github_repo") or "").strip()


def check(components=("plugin", "bot"), verbose=True):
    repo = _repo()
    if not repo:
    ...
    try:
        rel = _latest_release(repo, token)
        ...
        return None
    if not rel:
        print("No release found for %s." % repo)
        return None
    ...
    out = {"repo": repo, "release": rel, "latest": latest,
...
def update(components=("plugin",), do_deploy=False, do_apply=False, allow_unsigned=False):
    info = check(components, verbose=False)
    if not info:
        return
    ...
    for comp in components:
        ...
        if not st.get("in_release"):
            print("- %s: not in the latest release — skipping." % comp)
            continue
        if not st.get("newer"):
