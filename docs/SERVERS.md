...
Opt in and your server shows up in a public list other players can browse. It publishes a tiny
JSON file for your server with **name, region, and plugin version only**. Never IPs, SteamIDs, player
names, or live status. It is opt-in and you can turn it off any time.

Nuclear Option has no direct-connect, so the directory is there for discovery. Players find your
server by **name** (and region) in the in-game server browser.
...
- Pick an expiry you're happy with. Regenerate and re-paste it when it lapses.
- Generate, then copy the token (starts with `github_pat_…`).

**3. Add it to `run.bat`** (same place as your SFTP password, so it stays on your machine):
```bat
...
```
The token only grants write to that one repo. Keep it in `run.bat` (gitignored) and **never commit it**.
...
domain. It reads only the public JSON, so no tokens and nothing secret.
...
banner to show. Only the public banner image and a link to your gamemonitoring page are shown. On the
