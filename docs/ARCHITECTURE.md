...
> **Plugin version:** `anz.nukestats` `0.9.23` (the `[BepInPlugin]` attribute is authoritative).
...
```
        ...
        │   │                                                            │
        │  ┌┴───────────────────────────┐                                │
        │  │  NukeStats plugin v0.9.23   │  Debug.Log("[NOSTATS] {...}")  │
        │  │  (BepInEx)                  ├───────────────┐                │
        │  └─────────────▲──────────────┘               │                │
        │   reads:        │ reads/deletes:               ▼                │
...
The plugin is a single `BaseUnityPlugin` (`[BepInPlugin("anz.nukestats", "NukeStats", "0.9.23")]`). Every behaviour is a HarmonyLib patch or a `[Server]`-API call. All tunables live in `BepInEx/config/anz.nukestats.cfg`.
