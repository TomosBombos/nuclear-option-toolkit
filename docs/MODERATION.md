...
- **There are two separate ban lists.**
  - **Plugin ban list** (`plugin_bans.txt`) is written and enforced by the NukeStats plugin. That covers
    the teamkill ladder, the anti-grief hard-ban, and the Reports-tab Ban button. The plugin kicks a
    listed SteamID on sight.
  - **Game-native ban list** (`ban_list.txt`) is the game's own list. The `banlist-add` and
    `banlist-remove` commands drive it, and the game enforces it. It works even with nobody online.

  The web command centre keeps both in sync for explicit owner actions. The *automated* ladders only
  write the plugin list.

- **Plugin-side actions need a player online to take effect.** While the server is empty the plugin's
  tick driver idles. A plugin ban, kick, or setting relays but doesn't *apply* until someone joins. The
  game-native `banlist-add` and votekick aren't subject to this.
...
**same faction** as the dead unit, and not the victim's own aircraft. Using the top damager rather than
the last hit assigns blame correctly.

**The escalation ladder** (counted per match, per SteamID):

| Offence | Action |
|---|---|
| **1st** | **Eject** from the aircraft (life-neutral, so it does *not* count as a death against your skill rating) plus a private "first warning". |
| **2nd** | Rank reset to 0 on next sight, a private "next one is a BAN", and a short-delayed **kick**. |
| **3rd+** | Added to the **persistent ban list** (`plugin_bans.txt`) and kicked. |
...
This protects against a single connection mass-commanding units (a held key, macro, or UI loop) and
flooding the network buffer. That used to mass-disconnect the **entire lobby**. The goal is to kick
**only the one offender**. Never the lobby, and never a legitimate base-builder. It uses **two
cooperating detectors** (the "two-factor, misfire-safe" design), plus the buffer-protection layers below
that absorb the flood regardless.
...
| `Grief.RequireActiveFlooding` | **on** | **Two-factor safety.** When on, it kicks only on *sustained move-order flooding* and ignores the raw unit count. When off, it also trips on merely owning more than the threshold (more aggressive, can false-positive a base-builder). |
| `Grief.FloodOrdersPerSec` | **3** | The order rate (per second) that counts as flooding. Must be held ~4 seconds. A legitimate commander is well under 1/s, and the game caps accepted orders at ~5/s, so 3/s sustained means a macro. |
...
| `Grief.ReportOnly` | **off** | Detect and file a report but **don't kick**. Use this to validate the threshold for a session before enabling kicks. |
...
hard-bans, and queues a short-delayed kick. A re-act throttle stops the same player being punished twice
for one event.
...
dozens per second. Configured in `grief_flood.json` (edit, then restart the bot):

| Setting | Default | Meaning |
|---|---|---|
| `enabled` | on | Master enable. |
| `action` | `kick` | `kick` (recoverable), `ban` (plugin ban), or `report` (detect-only). |
| `drops_per_window` / `window_sec` | `30` / `3.0` | Trip when one SteamID exceeds 30 dropped commands in 3 seconds (~25/s over the cap, which is macro only). |
| `cooldown_sec` | `30` | Don't re-act on the same SteamID within this window. |
| `exempt_admins` | on | Never auto-act on an admin. |

When it trips it adds a report and kicks, bans, or logs per `action`, writing an `AUTO-KICK`,
`AUTO-BAN`, or `FLOOD REPORT` line to the activity feed.
...
- The bot path requires a **sustained storm of the game's own dropped-command lines**. A normal player
  generates about none.
- Admins are exempt by default in both. A cooldown stops repeated punishment for one event. Both default
  to a **recoverable kick** rather than a ban, and a report-only mode lets you validate thresholds
...
- **Layer B — `Flood.DropDeadNetIdRpcs`** (on): silently drops commands aimed at a dead or unknown unit
  (a log and allocation amplifier under a flood).
...
| **Enforced by** | the plugin kicking on sight (needs the plugin loaded plus a player online) | the game itself (works with nobody online) |
| **Scope** | permanent until removed; survives restarts | permanent until removed |

All bans here are **SteamID-based and permanent**. There is no built-in timed ban and no stored reason.
The split is why the **Reports-tab Ban is the most thorough**: it writes *both* lists and works even with
the server empty. The **player-popup Ban writes only the game list**, and the **automated ladders write
only the plugin list**.
...
- **Reports / Moderation tab → Ban:** bans on **both** lists at once (plugin and game), so it's immediate
  and survives an empty server. **Unban** reverses both. SteamIDs are validated before relaying.
- **Banned-players tab:** merges both lists (each row tagged plugin or game), with an **Unban** button and
  a free-text "Unban a SteamID" box. Use the box to clear a ban that isn't shown in the file listing yet
  (for example a fresh vote-kick auto-ban held in memory by the game).

---

## 4. Votekick (the game's built-in player vote-to-kick)

This is the **game's own** feature, configured in `DedicatedServerConfig.json` under `VoteKick`. It's
...
**Enable or disable it** with `run.bat --set-votekick on|off`. That surgically flips only
`VoteKick.Enabled` with a full safety check (it re-parses the config and aborts the upload if anything
else moved), keeps a backup, and reloads the config without a full restart. It's also exposed in the
⚙ Settings menu.
...
**Reports.** Shows anti-grief auto-kick and flag events from both detectors, each with the time, player,
SteamID, reason, units owned, order rate, and the action taken (rendered **banned**, **kicked**, or
**flagged**). Per row: **Ban** (confirm-gated, bans on both lists) or **Unban**, and **Clear** (remove
one). **Clear all** wipes the list. All actions route through the bot (the single writer of the reports
file) so cleared reports don't reappear.
...
Beyond this tab, the **⚙ Settings → Anti-Grief** group exposes every `Grief.*` knob live. You can flip
`RequireActiveFlooding`, set `ReportOnly` for a validation night, raise `FloodOrdersPerSec` if you see
false positives, or enable `HardBan`. **⚙ Settings → Moderation** holds `Teamkill.Enforce` and the
votekick toggle. If you make a setting *more lenient* than recommended, the UI warns you. It doesn't stop
you, but it can allow command-flooding.

---

## 6. New-joiner & squad protections

These don't punish anyone. They govern **who PvP auto-balance is allowed to move**, so the system doesn't
repeatedly yank new players or pre-made friend groups across teams. Balance moves are *life-neutral*
(they never hurt a skill rating) and PvP-only.
...
only when a side is *more than* this many ahead), `Balance.MinPlayers` (default 6, so small lobbies are
left alone), `Balance.WarnSeconds` (default 300, a 5-minute warning before any move), `Skill.BalanceBySkill`
...
| `Grief.ReportOnly` | off | yes | Detect and report, no kick (validation mode). |
